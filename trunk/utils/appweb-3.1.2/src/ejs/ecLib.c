    ...
    if (lookup->slotNum >= 0) {
        ...
        if (ejsIsType(np->lookup.obj)) {
...
#if UNUSED
                if (!cp->bind && !type->compiling && !type->block.obj.var.builtin) {
                    /*
                     *  Unbind references to non-core types that are not being compiled in this run. 
                     *  Checksums enable us to support this.
                     */
                    lookup->slotNum = -1;
                }
#endif
            ...
            if (block->nobind) {
                ...
                lookup->slotNum = -1;
#if UNUSED
            } else if (!cp->bind || block->nobind) {
                if (state->currentClass == 0 || (state->currentClass->instanceBlock != (EjsBlock*) lookup->obj)) {
                    lookup->slotNum = -1;
                }
#endif
            }
...
static void processModule(EcCompiler *cp, EjsModule *mp)
{
    ...
    if (! cp->outputFile) {
        if (mp->version) {
            ...
            path = mprJoinPathExt(cp->state, mp->name, EJS_MODULE_EXT);
        }
...
{
    if (ejsLoadScriptLiteral(ejs, path, EC_FLAGS_RUN | EC_FLAGS_NO_OUT | EC_FLAGS_DEBUG | EC_FLAGS_BIND) < 0) {
        return 0;
    }
    return ejs->result;
}
...
#if UNUSED
            //  TODO -- flags currently not used
            flags = cp->nobind ? 0 : EJS_MODULE_BOUND;
            rc += ecEncodeByte(cp, flags);
#endif
            if (rc < 0) {
                return MPR_ERR_CANT_WRITE;
            }
...
static EcNode *parseFunctionExpression(EcCompiler *cp)
{
    ...
    if (peekToken(cp) == T_GET || cp->peekToken->tokenId == T_SET) {
        ...
        field = createNode(cp, N_FIELD);
        field->field.fieldKind = FIELD_KIND_FUNCTION;

        funRef = createNode(cp, N_QNAME);
        funRef->qname = ejsCopyName(funRef, &fp->qname);
        field->field.fieldName = linkNode(np, funRef);
    }
#endif
