...
/****************************** Control Callbacks ****************************/
/*
 *  TODO - These routines should be shared across all gateways
 */
/*
 *  Define params[]
 */
...
static EjsVar *getRequestVar(void *handle, int field)
{
    ...
    case ES_ejs_web_Request_cookies:
        /*
         *  TODO - this should be cached like web->files below. CreateCookies should not set web->cookies
         */
        if (req->cookie) {
...
#if UNUSED
    case EJS_WEB_HOST_VAR:
        return setHostVar(handle, field, value);
    case EJS_WEB_REQUEST_VAR:
        return setRequestVar(handle, field, value);
#endif
    case EJS_WEB_RESPONSE_VAR:
