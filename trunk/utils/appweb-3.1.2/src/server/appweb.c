...
#if BLD_FEATURE_EJS
static void createEjsAlias(Mpr *mpr, MaHttp *http, MaServer *server, cchar *ejsAlias, cchar *homeDir);
#endif
#if BLD_UNIX_LIKE
...
MAIN(appweb, int argc, char **argv)
{
    ...
    char        *configFile, *ipAddr, *homeDir, *timeText;
...
#if BLD_FEATURE_EJS
    if (ejsAlias) {
        createEjsAlias(mpr, http, server, ejsAlias, homeDir);
    }
#endif
...
static void createEjsAlias(Mpr *mpr, MaHttp *http, MaServer *server, cchar *ejsAlias, cchar *homeDir)
{
    ...
    if ((path = strchr(ejsAlias, ':')) != 0) {
        *path++ = '\0';
        path = mprJoinPath(mpr, homeDir, path);
        path = mprStrcat(path, -1, path, "/", NULL);
    } else {
        path = mprStrcat(mpr, -1, homeDir, "/", NULL);
    }
