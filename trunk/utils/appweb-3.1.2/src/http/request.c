...
#if BLD_DEBUG && FUTURE
        if (conn->request && packet && packet->content) {
            if (conn->request->remainingContent == 0 && mprGetBufLength(packet->content) > 0) {
                mprLog(conn, 5, "Client is using pipelining");
            }
        }
#endif
    }
...
static bool parseHeaders(MaConn *conn, MaPacket *packet)
{
    ...
    for (count = 0; content->start[0] != '\r'; count++) {
        ...
        mprStrUpper(key);
        //  TODO OPT - why do this??? No needed for Ejs anymore
        for (cp = key; *cp; cp++) {
            if (*cp == '-') {
                *cp = '_';
            }
...
static void reportFailure(MaConn *conn, int code, cchar *fmt, va_list args)
{
    ...
    if (resp == 0 || req == 0) {
        ...
        /* 711 is a custom error used by the test suite. TODO - remove this */
        if (code != 711) {
