...
#if BLD_DEBUG
    if (strncmp(resp->mimeType, "image/", 6) == 0 || strcmp(resp->mimeType, "text/css") == 0) {
        putFormattedHeader(conn, packet, "Expires", req->host->expiresDate);
    }
#endif
...
void maRedirect(MaConn *conn, int code, cchar *targetUri)
{
     ...
     *  TODO - refactor this and have conn->abbreviateRequest flag instead.
     */
