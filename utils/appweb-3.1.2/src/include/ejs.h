 ...
 *  TODO - REFACTOR split attributes into type, function and property attributes.
 */
/*
...
#define EJS_ATTR_OPER_OVERLOAD          (1 << 5)                /**< Type overloaded operators (UNUSED) */
 ...
 *      ejsSetPropertyTrait ejsSerialize ejsDeserialize ejsParseVar ejsSerialize
 */
typedef struct EjsVar {
    ...
    #define ejsSetFmtDebugName(vp, fmt, arg) \
        if (1) { \
            mprSetName(vp, mprAsprintf(vp, -1, fmt, arg)); \
            ((EjsVar*) vp)->name = (char*) mprGetName(vp); \
        } else 
#endif
    #else
...
extern EjsEx *ejsAddException(EjsFunction *mp, uint tryStart, uint tryEnd, struct EjsType *catchType,
    uint handlerStart, uint handlerEnd, int numBlocks, int flags, int preferredIndex);
...
typedef struct EjsFrame {
    ...
    EjsVar          **stackBase;            /**< Start of the eval stack for this function */
...
typedef struct EjsTypeHelpers
{
    ...
    EjsVar  *(*serializeVar)(Ejs *ejs, EjsVar *vp);
...
typedef EjsVar  *(*EjsSerializeVarHelper)(Ejs *ejs, EjsVar *vp);
